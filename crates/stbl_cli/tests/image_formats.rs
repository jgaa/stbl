...
fn alpha_fallback_uses_png() {
    ...
    let scaled = out_dir.join("artifacts/images/_scale_720/alpha.png");
    assert!(scaled.exists(), "missing scaled alpha.png fallback");
    let html = read_page(&out_dir, "index.html");
    assert!(html.contains("artifacts/images/_scale_720/alpha.png"));
